...
def build_live365(timeline, show_title):
    ...
    for item in timeline:
        ...
        if ts == '00:00:00.000':
            ts = '00:00:00.001'
        album = item.get('album') or f"{item['title']} (Single)"
        markers.append({
            'offset': ts, 'media_type': 'music', 'title': item['title'],
            'artist': item['artist'], 'album': album, 'year': item.get('year', '')
        })

    def ms(ts):
        h, m, s = ts.split(':'); sec, milli = s.split('.')
        return int(h)*3600000 + int(m)*60000 + int(sec)*1000 + int(milli)

    limits = [(600000,5), (900000,7), (1800000,13), (4680000,33)]
    accepted = []
    corrections = 0
    for marker in markers:
        if marker['media_type'] == 'talk':
            accepted.append(marker); continue
        t = ms(marker['offset'])
        violates = any(
            sum(1 for x in accepted if x['media_type']=='music' and 0 <= t-ms(x['offset']) <= window) >= maximum
        ...
        if not violates:
            ...
            for prev in reversed(accepted):
                if prev['media_type'] == 'music':
                    prev.update({'media_type':'talk','title':show_title,'artist':show_title,'album':'','year':''})
                    corrections += 1
                    break
...
def process_job(job_id, audio_path, show_title, workdir):
    try:
        ...
        markers, corrections = build_live365(timeline, show_title)
        ...
        JOBS[job_id].update({
            ...
            'corrections': corrections, 'warnings': len(ambiguous),
...
def analyze():
    ...
    show_title = (request.form.get('show_title') or '').strip()
